...
TRANSLATIONS = {
    "en": {
        ...
        "btn_rebuild_force":     "Re-run Rebuild",
        ...
        "status_rebuilding":     "Rebuilding from Detected/...",
        "rebuild_done":          "Rebuild complete -> Rebuilt/{name}",
        "rebuild_failed":        "Rebuild failed: {err}",
        "rebuild_confirm_title": "Re-run Rebuild",
        "rebuild_confirm_msg":   "Re-run intensity-guided rebuild. Source: current edited mask (Labeling/) if it exists, else original Detected/ mask. The current mask will be replaced with the result. Continue?",
    },
    "zh": {
        ...
        "status_rebuilding":     "正在从 Detected/ 重建...",
        "rebuild_done":          "重建完成 -> Rebuilt/{name}",
        "rebuild_failed":        "重建失败: {err}",
        "rebuild_confirm_title": "重新重建",
        "rebuild_confirm_msg":   "重新执行强度拟合。输入: 当前已编辑的 mask(Labeling/ 中,若存在)或原始 Detected/ mask。当前 mask 会被结果替换。继续?",
    },
    "ko": {
        ...
        "status_rebuilding":     "Detected/ 에서 빌드 중...",
        "rebuild_done":          "빌드 완료 -> Rebuilt/{name}",
        "rebuild_failed":        "빌드 실패: {err}",
        "rebuild_confirm_title": "다시 빌드",
        "rebuild_confirm_msg":   "강도 기반 빌드를 다시 실행합니다. 입력: 편집된 마스크(Labeling/, 있는 경우) 또는 원본 Detected/ 마스크. 현재 마스크가 결과로 교체됩니다. 계속하시겠습니까?",
    },
}
