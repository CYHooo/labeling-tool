...
전체 흐름: **연결 마법사(V1 다운로드) → 라벨링 → EC2 업로드(V2→V3→V4)**
...
### 1) 연결 / 데이터 가져오기 (시작 시 팝업)

- `BASE URL`, `X-Viewer-Api-Key`, `sessionId` 를 입력합니다.
  (BASE/Key 는 최초 성공 후 `labeling_tool/config.json` 에 저장되어 다음부터 자동 입력됩니다.)
...
- 이미 받은 세션은 **「이미 받은 세션 열기」** 로 오프라인에서 바로 열 수 있습니다.
