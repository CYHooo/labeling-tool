...
- **스케일(px/cm)**: 서버가 사진별로 미리 계산해 둔 **PPM(px/cm)** 을 가져와 그대로 사용합니다
  (촬영 단계에서 ArUco 마커는 더 이상 쓰지 않습니다). 값을 보정하려면 **「수동 측정」** →
  기준선 양 끝 2점 클릭 → 실제 길이 입력으로 덮어쓸 수 있으며, 수동 스케일과 그 기준선은 저장되어
  재방문 시 복원되고 **서버 PPM 보다 우선**합니다. (서버 PPM 이 없을 때만 ArUco 자동검출로 폴백.)
...
위의 생산용 도구(`labeling_tool/`)와는 작은 경로 헬퍼(`labeling_tool.core.app_paths`)만 공유하며,
같은 실행 스크립트(`run.bat` / `run.sh`)와 로그인 화면에서 실행합니다.
